...
namespace autolabor_driver {
    ...
    void Pm1Driver::handle_canbus_msg(const chasu::CanBusMessage::ConstPtr &msg) {
        if((msg->node_type == CANBUS_NODETYPE_ECU) && (!msg->payload.empty())){
            ...
            // ROS_INFO("rec:vl: %f\n",current_vel_l);
            // ROS_INFO("rec:vr: %f\n",current_vel_r);
            chasu::CanBusService srv;
            srv.request.requests.clear();
            chasu::CanBusMessage target_speed;
            target_speed.node_type = 2;
            target_speed.payload.clear();
            for(int i = 0;i < 8;i++){
                target_speed.payload.push_back(0);   
            }
            target_speed.payload[0] = (unsigned char) current_vel_l * 360;
            target_speed.payload[1] = (unsigned char) current_vel_r * 360;
            if((current_vel_l + current_vel_r) >= 0){
                target_speed.payload[2] = 1;
            }
            else if((current_vel_l + current_vel_r) < 0){
                target_speed.payload[2] = 2;
            }
    ...
    void Pm1Driver::run() {
        ...
        canbus_client_ = node.serviceClient<chasu::CanBusService>("canbus_server");
