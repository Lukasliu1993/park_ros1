...
bool imu_checksum(uint8_t * payload){
  ...
  return checksum == ((payload[39] << 8) | payload[38]);
}
  ...
  if (serial.isOpen()){
    while(1){
      ...
      case 0:{ // Header 1
        ...
        ROS_INFO("header4\n");
        size_t read_payload_size = serial.read(payload, 40);
        state = read_payload_size == 40 ? 5 : 0;
        break;
      }case 5:{ // CHECKSUM
        ROS_INFO("header5\n");
