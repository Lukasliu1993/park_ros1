...
#include <fcntl.h>
#include <pthread.h>
#include <can_bus/dzt_p1_service.h>
#include <stdlib.h>
#include "can_bus/cmd2.h"
...
namespace autolabor_driver {
    ...
    bool CanbusDriver::verify_frame(CAN_OBJ *can,can_bus::CanBusMessage *msg)
    {      
    ...
    bool CanbusDriver::canbus_service(can_bus::CanBusService::Request &req, can_bus::CanBusService::Response &res) {
        if (req.requests.size() > 0) 
        {
            for (int i = 0; i < 1; i++) 
            {
            can_bus::CanBusMessage msg = req.requests[i];
            CAN_OBJ can;
            memset(&can, 0, sizeof(CAN_OBJ));
            int is_data = 0;
    ...
    void CanbusDriver::run() {
        ...
        if (init()) {
            ROS_INFO(" start ok\n");
            qianjin_pub = node.advertise<can_bus::cmd2>("/qianjin_pub", 10);
            jiaodu_pub = node.advertise<can_bus::cmd2>("/jiaodu_pub", 10);
            qianjin_rec = node.advertise<can_bus::cmd2>("/qianjin_rec", 10);
            jiaodu_rec = node.advertise<can_bus::cmd2>("/jiaodu_rec", 10);
            brush_pub = node.advertise<can_bus::cmd2>("/brush_pub", 10);
            canbus_msg_pub1_ = node.advertise<can_bus::CanBusMessage>("canbus_msg", 10);
