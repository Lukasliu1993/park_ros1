...
namespace autolabor_driver {
    ...
    private:
        ...
        bool canbus_service(can_bus::CanBusService::Request &req, can_bus::CanBusService::Response &res);

        bool trans_msg(int dev, int num, int ID, int externflag, vector <unsigned char> & data);
        bool opendev();
        void closedev();
        void startdev();
        bool verify_frame(CAN_OBJ *can,can_bus::CanBusMessage *msg);
    private:
        int gDevType1;
        int gDevIdx1;
        int gChMask1;
        ...
        int gTxType2; 
        int parse_rate_;
        ros::Publisher canbus_msg_pub1_,qianjin_pub ,qianjin_rec ,jiaodu_pub ,jiaodu_rec ,brush_pub ;
