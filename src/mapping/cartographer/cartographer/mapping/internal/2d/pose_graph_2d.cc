...
void PoseGraph2D::AddTrajectoryIfNeeded(const int trajectory_id) {
  ...
  if (!global_localization_samplers_finishedsubmap[trajectory_id]) {
    global_localization_samplers_finishedsubmap[trajectory_id] =
        absl::make_unique<common::FixedRatioSampler>(
            options_.global_sampling_ratio_finishedsubmap());
  }
}
...
void PoseGraph2D::ComputeConstraintForFinishedsubmap(const NodeId& node_id,
                                    const SubmapId& submap_id) {
  bool maybe_add_local_constraint = false;
  bool maybe_add_global_constraint = false;
  const TrajectoryNode::Data* constant_data;
  const Submap2D* submap;
  {
    ...
    if (node_id.trajectory_id == submap_id.trajectory_id ||
      ...
      // local search window.
      maybe_add_local_constraint = true;
    } else if (global_localization_samplers_finishedsubmap[node_id.trajectory_id]->Pulse()) {
      maybe_add_global_constraint = true;
    }
    constant_data = data_.trajectory_nodes.at(node_id).constant_data.get();
    submap = static_cast<const Submap2D*>(
...
WorkItem::Result PoseGraph2D::ComputeConstraintsForNode(
  ...
  // }
  for (const auto& submap_id : finished_submap_ids) {
    const SubmapId new_submap_id = submap_ids.front();
    const int delta = 1;
    if (new_submap_id.submap_index - submap_id.submap_index > delta || node_id.trajectory_id != submap_id.trajectory_id) {
      ComputeConstraint(node_id, submap_id);
    }
  }
  ...
  //       ComputeConstraintForFinishedsubmap(node_id, newly_finished_submap_id);
  //     }
  //   }
  // }
  if (newly_finished_submap) {
    ...
    for (const auto& node_id_data : optimization_problem_->node_data()) {
      ...
      for (int i = 1; i <= delta; i ++) {
        if (newly_finished_submap_id.submap_index < 1) {
          continue;
        }
        SubmapId near_submap_id = newly_finished_submap_id;
        near_submap_id.submap_index = newly_finished_submap_id.submap_index - i;
        if (data_.submap_data.at(near_submap_id).node_ids.count(node_id) > 0) {
          flag = true;
          continue;
        }
      }
